...
<div>
    ...
    <section v-else>
        <div class="mb-3">
            ...
            <select class="form-select from-select-lg"  name="" id="" v-model="name" @change="getUrl(name)">
        ...
        <div class="container ms-auto grid lg:grid-cols-4 gap-4 m-3">
            <div v-for="(p, index) in data.meals" :key="index">
                <nuxt-link :to="`products/${p.idMeal}`">
                    <product-components :product="p" />
                </nuxt-link>
            </div>
        </div>
    </section>
